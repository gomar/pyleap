...
def conditionning_algo(f, f1, algo_choice):
    ...
    # computing hb source term imag on real
    Dt = hb_computation.ap_source_term()
    Dt_imag = Dt + np.conjugate(Dt)
    Dt_real = Dt - np.conjugate(Dt)
    ratio = np.linalg.norm(Dt_imag) / np.linalg.norm(Dt_real)
    return hb_computation.conditionning(), ratio
...
def main(algo_choice, sampling, f1):
    ...
    result[:, 0], result[:, 1] = vfunc(f, f1, algo_choice)
    sorting = np.argsort(result, axis=0)[:,0]
    result[:, 0] = result[:, 0][sorting]
    result[:, 1] = result[:, 1][sorting]
    ...
    np.savetxt(fname='CONDITIONNING_IMAG.dat', X=result)
